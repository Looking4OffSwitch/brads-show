...
class QAAgent(BaseAgent):
    ...
    def __init__(
        ...
        """Initialize QA agent."""
        super().__init__(AgentRole.QA, config, llm)

    def get_system_prompt(self) -> str:
        """Get the QA Agent's system prompt."""
        return """# QUALITY ASSURANCE AGENT - FINAL VALIDATION
...
- No "figure it out later" gaps"""
    ...
    def get_task_instructions(self, task_type: str, context: AgentContext) -> str:
        """Get task-specific instructions for the QA Agent."""
        instructions = {
            "final_validation": self._final_validation_instructions(context),
            "quick_check": self._quick_check_instructions(context),
        }

        return instructions.get(
            task_type,
            f"Execute the '{task_type}' task with thorough quality assessment.",
        )

    def _final_validation_instructions(self, context: AgentContext) -> str:
        """Instructions for final validation."""
        return """## TASK: FINAL VALIDATION
...
**STATUS:** QA validation complete - [Gate decision]"""
    ...
    def _quick_check_instructions(self, context: AgentContext) -> str:
        """Instructions for a quick quality check."""
        return """## TASK: QUICK QUALITY CHECK
