"""
Senior Writer B - Dialogue and punch-up specialist.

Senior Writer B excels at writing punchy, quotable dialogue with high joke
density, executing punch-up passes to maximize laughs, and creating callbacks
that compound humor.
"""
...
class SeniorWriterB(BaseAgent):
    """
    Senior Writer B - Dialogue & Punch-Up Specialist.
    ...
    def __init__(
        ...
        """Initialize Senior Writer B agent."""
        super().__init__(AgentRole.SENIOR_WRITER_B, config, llm)

    def get_system_prompt(self) -> str:
        """Get Senior Writer B's system prompt."""
        return """# SENIOR WRITER AGENT B - DIALOGUE & PUNCH-UP SPECIALIST
...
- Verbal comedy and character-specific speech patterns"""
    ...
    def get_task_instructions(self, task_type: str, context: AgentContext) -> str:
        """Get task-specific instructions for Senior Writer B."""
        instructions = {
            "generate_pitches": self._generate_pitches_instructions(context),
            "map_jokes": self._map_jokes_instructions(context),
            "draft_section": self._draft_section_instructions(context),
            "table_read_review": self._table_read_review_instructions(context),
            "punch_up": self._punch_up_instructions(context),
        }

        return instructions.get(
            task_type,
            f"Execute the '{task_type}' task using your expertise in dialogue and jokes.",
        )

    def _generate_pitches_instructions(self, context: AgentContext) -> str:
        """Instructions for generating dialogue-driven pitches."""
        return """## TASK: GENERATE DIALOGUE-DRIVEN PITCH CONCEPTS
...
**Specialty:** Dialogue-driven concepts with high joke density potential"""
    ...
    def _map_jokes_instructions(self, context: AgentContext) -> str:
        """Instructions for mapping joke density and rhythm."""
        return """## TASK: MAP JOKE DENSITY AND RHYTHM FOR STORY BREAKING
...
**STATUS:** Joke map ready for beat sheet synthesis"""
    ...
    def _draft_section_instructions(self, context: AgentContext) -> str:
        """Instructions for drafting a dialogue-heavy section."""
        return """## TASK: DRAFT DIALOGUE-INTENSIVE SCRIPT SECTION

You've been assigned a dialogue-heavy section. Write with maximum joke density and strong verbal comedy.

### Your Drafting Process
1. Review the beat sheet and joke map for your section
2. Write snappy, punchy dialogue
3. Include verbal comedy techniques (misdirection, rule of threes, etc.)
4. Set up callbacks for later payoff
5. Aim for 2-3+ laughs per page
6. Make every line earn its place

### Drafting Standards
- Punchlines should be short and surprising
- Set up expectations, then subvert
- Include tags (extra jokes after main punchlines)
...
**Target Joke Density:** [X laughs per page]
...
[Continue writing the full section]

---

**JOKE TRACKING:**
- Page 1: [List jokes/laugh moments]
- Page 2: [List jokes/laugh moments]
- Total laughs this section: [X]

**CALLBACK SETUPS:**
- [Element set up for later payoff]

**ALT JOKES (backups):**
- Line: "[Original]" → ALT: "[Alternative punchline]"

**STATUS:** Section draft complete, ready for assembly"""

    def _table_read_review_instructions(self, context: AgentContext) -> str:
        """Instructions for table read review focusing on jokes."""
        return """## TASK: TABLE READ REVIEW - JOKE/DIALOGUE FOCUS

Review the first draft focusing on your specialty: jokes and dialogue.

### Your Review Focus
- Is joke density sufficient (2-3+ per page)?
- Are punchlines landing (surprising, well-constructed)?
- Is dialogue rhythm working?
- Are callbacks placed effectively?
- Any jokes that fall flat?
- Where could we add more laughs?

## OUTPUT FORMAT

# TABLE READ NOTES - Senior Writer B (Dialogue/Jokes)

## Joke Density Assessment
**Current Density:** [X laughs per page average]
**Target Density:** 2-3+ per page
**Assessment:** [On target / Below target - needs punch-up]

## Page-by-Page Joke Review

### Page 1
**Jokes Present:** [List]
**Strongest:** [Which lands best]
**Weakest:** [Which needs work]
**Opportunities Missed:** [Where we could add a laugh]
...
## Punchline Quality Assessment
**Strong Punchlines:**
- Page X: "[Line]" - Why it works
- Page Y: "[Line]" - Why it works

**Weak Punchlines (Need Work):**
- Page X: "[Line]" - Problem: [too predictable, too long, etc.]
- Page Y: "[Line]" - Problem: [explanation]

## Callback Effectiveness
**Callbacks Present:**
- Setup page X → Payoff page Y: [Works / Needs work]

**Callback Opportunities Missed:**
- [Element that could be called back but isn't]

## Dialogue Rhythm
**Pacing Issues:**
- Page X: [Too slow / Too rushed / Just right]

## Punch-Up Recommendations

### CRITICAL:
- [Joke that must be fixed/added]

### IMPORTANT:
- [Joke improvements that would help]

### POLISH:
- [Small joke enhancements]

**STATUS:** Table read review complete (Joke/Dialogue focus)"""

    def _punch_up_instructions(self, context: AgentContext) -> str:
        """Instructions for punch-up pass."""
        return """## TASK: PUNCH-UP PASS ON DRAFT

Execute a comprehensive punch-up pass to maximize laughs.
